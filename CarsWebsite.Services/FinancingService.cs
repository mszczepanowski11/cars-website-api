...
public class FinancingService : IFinancingService
{
    ...
    public async Task<int> CreateInquiryAsync(CreateFinancingInquiryDto dto, int? userId)
    {
        ...
        // CTO audit Etap 4: durable Hangfire queue instead of a bare fire-and-forget task - each
        // notification is its own job so one failing (and retrying) never blocks or duplicates
        // the other.
        _backgroundJobClient.Enqueue<IFinancingService>(x => x.SendFinancingAdminNotificationAsync(inquiry.Id, advert.Title));
        _backgroundJobClient.Enqueue<IFinancingService>(x => x.SendFinancingUserConfirmationAsync(inquiry.Id, advert.Title));

        return inquiry.Id;
    }

    private static string BuildDetailsHtml(FinancingInquiry inq, string advertTitle, string typeLabel)
    ...
    public async Task SendFinancingAdminNotificationAsync(int inquiryId, string advertTitle)
    {
        var inq = await _db.FinancingInquiries.AsNoTracking().FirstOrDefaultAsync(i => i.Id == inquiryId);
        if (inq == null)
        {
            _logger.LogWarning("[Financing] SendFinancingAdminNotificationAsync: lead #{Id} no longer exists", inquiryId);
            return;
        }
        ...
        var adminHtml = EmailService.BuildHtml(
            ...
            detailsHtml: BuildDetailsHtml(inq, advertTitle, typeLabel),
            ctaUrl: advertUrl,
    ...
    public async Task SendFinancingUserConfirmationAsync(int inquiryId, string advertTitle)
    {
        var inq = await _db.FinancingInquiries.AsNoTracking().FirstOrDefaultAsync(i => i.Id == inquiryId);
        if (inq == null)
        {
            _logger.LogWarning("[Financing] SendFinancingUserConfirmationAsync: lead #{Id} no longer exists", inquiryId);
            return;
        }
        if (string.IsNullOrEmpty(inq.Email))
            return; // no address to confirm to - not every inquiry provides one
