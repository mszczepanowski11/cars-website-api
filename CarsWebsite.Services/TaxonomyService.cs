...
public class TaxonomyService : ITaxonomyService
{
    ...
    public TaxonomyService(AppDbContext context, IDistributedCache cache, ITaxonomyCacheVersion cacheVersion)
    {
        _context = context;
        _cache = cache;
    ...
    // version (see ITaxonomyCacheVersion.BumpAsync, called from AdminController) instead of trying
    // to remove every individual key, since the underlying cache has no prefix/wildcard eviction
    // and there are many independent cached shapes here (brands/models/generations/engines/trims/
    // features/etc). Old-versioned entries just age out on their own TTL instead of being
    // reachable again.
    private async Task<string> KeyAsync(string suffix) => $"taxonomy:v{await _cacheVersion.GetCurrentAsync()}:{suffix}";

    public async Task<IEnumerable<Brand>> GetFullTaxonomyAsync()
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("full"), async entry =>
        {
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("brands"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
            return await _context.Brands
                .AsNoTracking()
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync($"brands:category:{categoryId}"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
            return await _context.Brands
                .FromSqlRaw(@"
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync($"generations:model:{modelId}"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync($"engines:generation:{generationId}"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("fueltypes"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("gearboxes"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("bodytypes"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("drivetypes"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("colors"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("features"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("vehiclecategories"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("featurecategories"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync($"featurecategories:vehicle:{vehicleCategoryId}"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
            return await _context.FeatureCategories
                .AsNoTracking()
    ...
    {
        var cacheKey = await KeyAsync($"featurecategories:context:{vehicleCategoryId}:{brandId}:{modelId}");
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync($"trims:generation:{generationId}"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync($"engines:trim:{trimId}"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
            return await _context.EngineVersions
                .AsNoTracking()
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync($"enginespecs:{engineVersionId}"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
            return await _context.EngineVersions
                .AsNoTracking()
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync($"vehiclesubtypes:category:{vehicleCategoryId}"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync("partcategories"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
    ...
    {
        return await _cache.GetOrCreateAsync(await KeyAsync($"partsubcategories:category:{partCategoryId}"), async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheDuration;
